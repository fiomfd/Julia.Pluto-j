...
begin
	...
	P=zeros(2)
	Q=zeros(2)
	P[1]=-tan(θ*pi/180)/sqrt(1+tan(θ*pi/180)^2)
	Q[1]=1/sqrt(1+tan(θ*pi/180)^2)
	P[2]=tan(θ*pi/180)/sqrt(1+tan(θ*pi/180)^2)
	Q[2]=-1/sqrt(1+tan(θ*pi/180)^2)
	implicit_plot(f;
				  grid=false,
				  xlim=(-2,2),
				  xlabel=L"x",
				  ...
				  legend=false, 
				  linewidth=2)
	plot!([-2,2],[-2*tan(θ*pi/180)+sqrt(1+tan(θ*pi/180)^2),
				  2*tan(θ*pi/180)+sqrt(1+tan(θ*pi/180)^2)],
	 linecolor=:blue,
	 linewidth=2,
	 legend=:false)
	plot!([-2,2],[-2*tan(θ*pi/180)-sqrt(1+tan(θ*pi/180)^2),
...
# ╔═╡ 27a77ae0-f320-4378-8f62-6e71b15bedf5
begin
	p1=zeros(1)
	q1=zeros(1)
    p1[1]=501+100*a1
...
manifest_format = "2.0"
project_hash = "17b90707119aa940c9873fe05aa9583bc1f761ea"
