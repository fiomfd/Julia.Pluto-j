...
- The computational thinking is the idea to actively use mathematics and computers to visualize and solve problems.


- Today, universities around the world offer courses titled "computational thinking" taught by faculty skilled in classical mathematics and programming languages, and it has become an essential part of modern education.


- [MIT 18S.191 Introduction to Computational Thinking](https://computationalthinking.mit.edu/Fall24/) is the most famous and the most excellent course on computational thinking in the world. 
...
md"""
#### Computational thinking about what the differentiablity is 
"""
...
区間$I$上の1変数関数 $f(x)$ のグラフ $\Gamma(f):=\{(x,f(x)) : x \in I\}$ 上の点 $(a,f(x))$, $a \in I$ を通る任意の傾き $\beta$ の直線 $y=g(x):=f(a)+\beta(x-a)$ を考えて, $f(x)$ と $g(x)$ のグラフの点 $(a,f(a))$ における近づき方 $f(x)-g(x)$ を観察する. 
"""
